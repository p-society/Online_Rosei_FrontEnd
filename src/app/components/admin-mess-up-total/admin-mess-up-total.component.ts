    ...
    if (Info.userType !== "admin") {
      ...
      this.authService.getUsersmessUp().subscribe(data => {
        if (data.data.success) {
          ...
          // BreakFast non-Veg
          this.monBrNonVegCounter = this.messUp.map(t => t.couponUpMess.map(val => val.messup.map((t, index) => (t.day === "Monday" && t.breakfast)).filter(val => { return val === "nonVeg" })[0])).
          ...
          // Lun non-Veg
          this.monLunNonVegCounter = this.messUp.map(t => t.couponUpMess.map(val => val.messup.map((t, index) => (t.day === "Monday" && t.lunch)).filter(val => { return val === "nonVeg" })[0])).
            filter(val => { return val[0] === "nonVeg" }).length;
          ...
          // Dinner non-Veg
          this.monDinNonVegCounter = this.messUp.map(t => t.couponUpMess.map(val => val.messup.map((t, index) => (t.day === "Monday" && t.dinner)).filter(val => { return val === "nonVeg" })[0])).
            filter(val => { return val[0] === "nonVeg" }).length;
